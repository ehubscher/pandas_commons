...
def clean_dataframe(df: DataFrame, drop_row_if_NaN: List[str] = None, index_col: str = None) -> DataFrame:
    """
    ...
    :param drop_row_if_NaN: List of column names that will have it's respective 
        row dropped if it contains a cell with a NaN value.
    ...
    if not type(df_cpy.index) is RangeIndex:  # is it the default index?
        df_cpy = reset_index(df_cpy)

    df_cpy.columns = Series(list(df_cpy.columns.values))

    try:
        df_cpy = df_cpy.dropna(how="all", subset=drop_row_if_NaN)
    ...
    if index_col and index_col in df_cpy.columns:
        if str() in df_cpy[index_col].values or numpy.nan in df_cpy[index_col].values:
            logger.warning(f"Index column {index_col} cannot be set. This column contains blank values.")
            logger.warning(
        ...
        if not df_cpy[index_col].duplicated().any():
            try:
                df_cpy.set_index(keys=index_col, drop=True, inplace=True, verify_integrity=True)
            except ValueError as e:
                logger.exception(f"Index column {index_col} cannot be set.\r\n{str(e)}")
        else:
            logger.warning(f"Index column {index_col} cannot be set. This column contains duplicates.")
            logger.warning(f"Column duplicates are at: {get_column_duplicates(column=df_cpy[index_col])}")
...
def merge_dataframes(dataframes: List[DataFrame]) -> DataFrame:
    ...
    for dataframe in dataframes[1:]:
        ...
        result = reset_index(df=result)

    return result


def reset_index(df: DataFrame) -> bool:
    """
